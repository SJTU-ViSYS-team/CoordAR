...
def calc_mssd_recall(TXO_pred, TXO_gt, points, symmetries, diameters):
    ...
    P_bar_S_points, P_hat_points = _transform_pts_using_sym(
        TXO_pred, TXO_gt, points, symmetries
    )
    ...
    threshholds = diameters.unsqueeze(1) * torch.linspace(
        0.05, 0.5, 10, device=DEVICE
    ).unsqueeze(0)
    mssd_recall = (mssd.unsqueeze(1) < threshholds).float().mean(dim=1)
    return mssd_recall


def calc_add_recall(TXO_pred, TXO_gt, points, symmetries, diameters):
    DEVICE, (B, N_SYM, _, _) = symmetries.device, symmetries.shape
    P_bar_S_points, P_hat_points = _transform_pts_using_sym(
        TXO_pred, TXO_gt, points, symmetries
    )
    l2_dist = (P_hat_points - P_bar_S_points).pow(2).sum(dim=-1).sqrt()
    l2_dist = l2_dist.view(B, N_SYM, -1)
    add = l2_dist.mean(dim=-1).min(dim=-1).values  # B
    mssd_recall = (add < 0.1 * diameters).float()
    return mssd_recall
...
def calc_mspd_recall(TXO_pred, TXO_gt, points, symmetries, K):
    ...
    P_bar_S_points_proj = _my_project_pts(
        K.repeat_interleave(N_SYM, dim=0), P_bar_S_points
    )
