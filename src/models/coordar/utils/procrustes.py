...
# modified from https://github.com/qinzheng93/GeoTransformer/blob/main/geotransformer/modules/registration/procrustes.py#L6
def weighted_procrustes(
    ...
    with torch.amp.autocast("cpu", dtype=torch.float32):
        with torch.amp.autocast("cuda", dtype=torch.float32):
            U, _, V = torch.svd(H.cpu().float())  # H = USV^T
