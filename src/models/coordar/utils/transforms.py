...
det = lambda t: np.linalg.det(t.detach().cpu().float().numpy())
...
# From Cosypose
def add_noise(TCO, euler_deg_std=[15, 15, 15], trans_std=[0.1, 0.1, 0.05], generator=None):
    TCO_out = TCO.clone()
...
@torch.no_grad()
def mat2SO3(mat: torch.Tensor) -> SO3:
    assert mat.shape[-2] == mat.shape[-1] == 3
    quat = mat2quat(mat)
    return SO3(quat)
